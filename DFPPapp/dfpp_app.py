...
# --- ページ設定 ---
st.set_page_config(page_title="DFPP Sim Ver.33", layout="wide")
st.title("🧮 DFPP Advanced Simulator Ver.33")
st.markdown("### グラフ・詳細解説 完全実装版")
...
補充Alb量    : {supplied_alb_g:.0f} g (不足: {abs(diff_g):.0f} g)
        ...
        try:
            st.image("dfpp_circuit.png", caption="DFPP回路図", width=400)
        except:
            st.warning("⚠️ 'dfpp_circuit.png' が見つかりません。")

        # -----------------------------------------------------
        # 📊 処理量シミュレーション (グラフ) - ★確実表示★
        # -----------------------------------------------------
        st.markdown("---")
