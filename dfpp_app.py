...
# --- ページ設定 ---
st.set_page_config(page_title="DFPP Sim Ver.36 信州上田医療センター腎臓内科", layout="wide")
...
補充Alb量    : {supplied_alb_g:.0f} g (不足: {abs(diff_g):.0f} g)
    ...
    with st.expander("⚗️ 3. SC (ふるい係数) と 阻止率 (RC)", expanded=True):
        st.markdown("""
        * **SC (Sieving Coefficient)**: 膜を「通り抜けて体に戻る」割合 ($0.0 \\sim 1.0$)。アルブミンSCの数値が大きいほど、体にもどるアルブミンが増えます。病因物質SCの値が大きい（分子量が大きい）程、除去効率が低下します。＝必要血漿処理量が増大します。
        * DFPPにおけるSCふるい係数は、単純血漿交換や選択的血漿交換とは意味合いが真逆になることに注意して下さい。（単純血漿交換では通り抜けた物質は廃棄される　DFPPでは通り抜けた物質が体に戻る）
        * **RC (Rejection Coefficient)**: 膜で「阻止されて廃棄される」割合 ($RC = 1 - SC$)。阻止率=（１－ふるい係数）
        """)
    ...
    with st.expander("⚠️ 4. カタログ値・他サイトとの乖離理由 (重要)", expanded=True):
        ...
        * **結果**: 
            * 本アプリの方がアルブミン喪失量（補充必要量）が多く算出されます。これは **補充不足による低血圧等のトラブルを防ぐため、安全サイド** の数値を出すように設計しているためです。旭化成の計算サイトと近づけるには、病因物質のSC=0.1 アルブミンSC=0.4前後が目安です。
            * 病因物質SCの初期値は、カタログ資料におけるIgGに対するふるい係数を参照しています。
