...
補充Alb量    : {supplied_alb_g:.0f} g (不足: {abs(diff_g):.0f} g)
    ...
    with st.expander("⚗️ 3. SC (ふるい係数) と 阻止率 (RC)", expanded=True):
        st.markdown("""
        * **SC (Sieving Coefficient)**: 膜を「通り抜けて体に戻る」割合 ($0.0 \\sim 1.0$)。アルブミンSCの数値が大きいほど、体にもどるアルブミンが増えます。病因物質SCの値が小さい程、除去効率が高まります。＝必要血漿処理量は減ります。
        * DFPPにおけるSCふるい係数は、単純血漿交換や選択的血漿交換とは意味合いが真逆になることに注意して下さい。
